...
tr:nth-child(even) {
 ...
 <c:forEach var="pr" items="${cart}"> 
  <tr>
    ...
    <td><c:out value="${pr.quantity}"></c:out></td>
    <td><c:out value="${pr.price}"></c:out></td>
    <td><c:out value="${pr.quantity*pr.price}"></c:out></td>
    <c:set var="total" value="${total+pr.quantity*pr.price}"></c:set>
    </tr>
 </c:forEach>
...
<a href="viewcart">${cart.size()}</a>
item(s) in cart
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

<c:if test="${empty uid}"><a href="login?cont=viewcart">sign in</a>
<a href="Login?cont=viewcart">placeorder</a>
</c:if>
<h2>${uid}</h2>
<c:if test="${not empty uid}"><a href="logout">Sign out</a>
<br>
<h1><a href="order">placeOrder</a></h1>
</c:if>
